			...
			if (TentativeProofFound_Flag)
			{
				...
				*/
				...
				for (const auto& Axiom : Axioms_UInt64Vec)
				{
					...
					const bool AxiomCallRouteFoundFlag =
						AxiomCallGraphMap[Theorem[guid_UInt64]][Axiom[guid_UInt64]] == true;
						/*
						( AxiomCallGraphMap.find(Theorem[last_UInt64]) != AxiomCallGraphMap.end() ) &&
						( AxiomCallGraphMap[Theorem[last_UInt64]].find(Axiom[guid_UInt64]) != AxiomCallGraphMap[Theorem[last_UInt64]].end() ) &&
						( AxiomCallGraphMap[Theorem[last_UInt64]][Axiom[guid_UInt64]] == true );
						*/

					if (!AxiomCallRouteFoundFlag)
						continue;
