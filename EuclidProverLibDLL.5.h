...
/*
  ...
  UPDATES (C++23)
    +Prime(k++) >> Prime([k++])
    +unordered_map ('key' == 'KEY') >> unordered_multimap ('key' != 'KEY')
  ...
  Usage Example.
    ...
    // Theorem (C++)
    std::vector<std::string>> InTheoremToProveStdStrVec  = { "{", "PlayerCharacterSideKick", "}", "IsIn", "{", "QuadUtilityVehicle", "}", "=", "{", "QuadUtilityVehicle", "}", "and", "{", "VehicleDriveDisabled", "}" };
...
std::unordered_map<std::string, uint64_t> SymbolToPrimeNumber_UInt64Map;
...
*/
uint64_t Prime(uint64_t InIndex_UInt64 = -1)
{
    const uint64_t& Index_UInt64 = InIndex_UInt64 > -1 ? InIndex_UInt64 : PrimeComposite_UInt64Vec.size();
    ...
    std::vector<std::string>>>& InAxioms_UInt64Vec)
{
    ...
    */
    ...
    () -> void
    {
        for (const std::vector<std::string>& Statement_StdStrVec : InTheorem_UInt64Vec)
        {
            ...
            for (const std::string& Symbol_StdStr : Statement_StdStrVec)
            {
                ...
                const auto& it = SymbolToPrimeNumber_UInt64Map.find (Symbol_StdStr);
                if (it != SymbolToPrimeNumber_UInt64Map.end())
                {
                    PrimeProduct_UInt64Vec *= it->second;
                } else {
                    // Symbol is not in the prime number map...
                    const uint64_t p = Prime ();
                    SymbolToPrimeNumber_UInt64Map.emplace (Symbol_StdStr, p);
        ...
        TempTheorem_UInt64Vec.emplace_back (1); // guid
    };
    ...
    std::unordered_multimap<uint64_t,bool>> 
    CallHistory_00 {},
    CallHistory_01 {},
    CallHistory_02 {},
    ...
    // Todo: Offload math operations onto the GPU
    // Todo: Support Remove and Disable operations on axioms
          ...
          ++TotalProofsFound_UInt64;
            ...
            for (const std::vector<uint64_t>& Axiom : Axioms_UInt64Vec)
            {
                if (
                    ...
                    CallHistory_00.find(Theorem[LHS])->second.find(Axiom[LHS]) == 
                    CallHistory_00.find(Theorem[LHS])->second.begin() */)
                {
                    CallHistory_00.emplace( Theorem[LHS], std::unordered_multimap<uint64_t, bool> {{Axiom[LHS], true}});
                ...
                if (
                    ...
                    CallHistory_01.find(Theorem[LHS])->second.find(Axiom[RHS]) ==
                    CallHistory_01.find(Theorem[LHS])->second.begin() */)
                {   
                    CallHistory_01.emplace( Theorem[LHS], std::unordered_multimap<uint64_t, bool> {{Axiom[RHS], true}});
                ...
                if (
                    ...
                    CallHistory_02.find(Theorem[RHS])->second.find(Axiom[LHS]) ==
                    CallHistory_02.find(Theorem[RHS])->second.begin() */)
                {
                    CallHistory_02.emplace( Theorem[RHS], std::unordered_multimap<uint64_t, bool> {{Axiom[LHS], true}});
                ...
                if (
                    ...
                    CallHistory_03.find(Theorem[RHS])->second.find(Axiom[RHS]) ==
                    CallHistory_03.find(Theorem[RHS])->second.begin()*/ )
                {
                    CallHistory_03.emplace( Theorem[RHS], std::unordered_multimap<uint64_t, bool> {{Axiom[RHS], true}});
